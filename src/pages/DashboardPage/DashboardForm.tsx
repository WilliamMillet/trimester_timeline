...
const CourseRatingForm: React.FC = () => {
  ...
  function updateWeeksToComplete(course: any, completionPercentage: number): any {
    ...
    if (courseCodes?.length > 0) {
      fetch("http://localhost:5000/api/courses/get-assignments-duration-data", {
        ...
        .then((assignmentsData) => {
          const updatedData = assignmentsData.map((course: any) => {
            const matchingCourse = data.courses.find(
              (c) => c.courseCode === course.courseCode
            );
            if (matchingCourse && matchingCourse.completionPercentage) {
              return updateWeeksToComplete(
                {
                  ...course,
                  ability: matchingCourse.skillLevel
                },
                matchingCourse.completionPercentage
              );
            }
            return {
              ...course,
              ability: matchingCourse ? matchingCourse.skillLevel : null
            };
          });
